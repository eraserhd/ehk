...
(define (char-vector-compare-1 v1 v2 p check)
  (let ((len1 (vector-length v1))
	(len2 (vector-length v2)))
    (define (p2 x y)
      (or (= x y) (p x y)))
    (let loop ((i 0))
      (cond ((>= i len1) (p len1 len2))
	    ((>= i len2) (p len2 len1))
	    ((p2 (char->integer (check v1 i))
		 (char->integer (check v2 i)))
	     (loop (+ i 1)))
	    (else #f)))))

(define (char-vector-compare p)
  (define (check v i)
    (let ((x (vector-ref v i)))
      (if (char? x) 
	  x
	  (%error "not a character vector" v))))
  (cut char-vector-compare-1 <> <> p check))

(define (char-vector-compare-ci p)
  (define (check v i)
    (let ((x (vector-ref v i)))
      (if (char? x) 
	  (char-downcase x)
